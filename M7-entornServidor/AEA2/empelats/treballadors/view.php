...
private $diccionari = array (
	...
	'subtitle' => array (
		...
		'form_delete' => 'Escull persona a Eliminar',
		'view_delete' => 'Taula despres eliminar'
	), 
    'capçalera' => array (
		'view1' => array('nom','edat'), // table headers for each view
        'view2' => array('nom','alcada'),
		'view_select' => array('nom','Cognoms','professio', 'telefon'),
		'view_insert' => array('nom','Cognoms','professio', 'telefon'),
		'view_update' => array('nom','Cognoms','professio', 'telefon'),
		'view_delete' => array('nom','Cognoms','professio', 'telefon')
	),
	'form' => array(
		'form_select' => array('nom'), //TODO añadir mas formularios (los que haga falta para el ejercicio)
		'form_insert' => array('nom', 'cognoms', 'profession', 'telefon'),
		'form_searchPerson' => array('nom'),
		'form_update' => array('nom', 'cognoms', 'profession', 'telefon'),
		'form_delete' => array('nom'),
	)
);
...
public function retornar_vista ($vista, $dades=array(), $message="Dades de l'usuari") {
	...
	if($vista=='form_select' || $vista == 'form_insert' || $vista == 'form_searchPerson' || $vista == 'form_update' || $vista == 'form_delete') {
		...
		if ($vista == 'form_delete'){
			$html = str_replace('{url}', "view_delete",$html);
		}
	...
	if ($vista=='view_select' && count($dades)>0 || $vista == 'view_insert' && count($dades)>0 || $vista == 'view_update' || $vista = 'view_delete') {
		// the view template is read and its contents is included in the main template
		$view = file_get_contents(__DIR__ . '/../site_media/html/view_template.html');
		$html = str_replace ('{main}', $view, $html);
